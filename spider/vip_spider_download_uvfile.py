...
class GoodsState(State):
    ...
    def do(self, driver):
        try:
            self.browser_operation(driver=driver, locate_way='find_element_by_xpath',
                                   xpath="//div[@id='compass-app-body']/div/div/div/ul/li[4]/div",
                                   operator='click')  # 点击：左栏第一级 商品
            time.sleep(0.5)
            self.browser_operation(driver=driver, locate_way='find_element_by_xpath',
                                   xpath="//div[@id='compass-app-body']/div/div/div/ul/li[4]/ul/li",
                                   operator='click')  # 点击：左栏第二级 商品详情
            time.sleep(0.5)
            self.browser_operation(driver=driver, locate_way='find_element_by_xpath',
                                   xpath="//div[@id='compass-app-body']/div[2]/div/div[2]/div/label[2]/span/span",
                                   operator='click')  # 点击：档期圆点
            time.sleep(0.5)
            # self.browser_operation(driver=driver, locate_way='find_element_by_xpath',
            #                        xpath="//div[@id='compass-app-body']/div[2]/div/div[3]/div/div/div/div/div[2]/div/div/label/span/span",
            #                        operator='click')  # 点击：全部方框
            # self.browser_operation(driver=driver, locate_way='find_element_by_xpath',
            #                        xpath="//div[@id='compass-app-body']/div[2]/div/div[3]/div/div/div/div/div[2]/div/div/label/span/span",
            #                        operator='click')  # 点击：全部方框
            self.browser_operation(driver=driver, locate_way='find_element_by_xpath',
                                   xpath="//div[@id='compass-app-body']/div[2]/div/div[3]/div/div/div/div/div[2]/div/div[4]/div/div/label/span/span",
                                   operator='click')  # 点击：流量方框
            time.sleep(0.5)
            self.browser_operation(driver=driver, locate_way='find_element_by_xpath',
                                   xpath="//div[@id='compass-app-body']/div[2]/div/div[3]/div/div/div/div/div[2]/div/div[2]/div/div[2]/span/label/span/span",
                                   operator='click')  # 点击：三级品类
            time.sleep(0.5)
            self.browser_operation(driver=driver, locate_way='find_element_by_xpath',
                                   xpath="//div[@id='compass-app-body']/div[2]/div/div[3]/div/div/div/div/div[3]/div/div/div/label/span/span",
                                   operator='click')  # 点击：条形码方框
            time.sleep(0.5)
...
def crawl_uvfile_data(login_user, password, download_path, crawl_days, crawl_dates, share_list=None):
    logger = log.getLogger('spider_uv')
    logger.info('start spider_uv')
    global LOGIN_USER, LOGIN_PASSWORD, CRAWL_DATES
    LOGIN_USER = login_user
    LOGIN_PASSWORD = password
    CRAWL_DATES = crawler_days(crawl_days, crawl_dates)

    browser_download_path = download_path + '\\uv'
    ...
    b.close()
    logger.info('end spider_uv')
# def download_and_process(user_id, shop, login_user, password,
#                          rawFileSavePath, csvSaveRootPath,crawlDays,crawlDates):
#     rawFileSavePath = rawFileSavePath + '\\uv'
...
#                 "//div[@id='compass-app-body']/div[2]/div/div[3]/div/div/div/div/div[2]/div/div/label/span/span").click()   #点击：全部方框
#             b.find_element_by_xpath(
#                 "//div[@id='compass-app-body']/div[2]/div/div[3]/div/div/div/div/div[2]/div/div[4]/div/div/label/span/span").click()    #点击：流量方框
#             b.find_element_by_xpath(
#                 "//div[@id='compass-app-body']/div[2]/div/div[3]/div/div/div/div/div[2]/div/div[2]/div/div[2]/span/label/span/span").click()    #点击：三级品类
#             time.sleep(TIMING)
#             b.find_element_by_xpath(
...
#                 {dangqi:b.find_element_by_xpath(
...
#     print('crawl dates:',crawlDates)
#     for i,brand in enumerate(brands):
#         # print('i %s,brand %s'%(i,brand))
#         try:
...
#             break
#
#     print('uv download end')
#     delete_duplicate_files(rawFileSavePath) # 删除重复下载文件
#     b.close()
#         # while True:
#         #     try:
#             # b.find_element_by_xpath(r'//*[@id="dangqi-query-sel"]').click() # 点击下拉菜单：请选择档期
#             # options = b.find_elements_by_xpath(r'//*[@id="dangqi-query-sel"]//option')
#
#         # if len(options) > 0:
#         #     dangQiOptions = [option for option in options]
#         # except Exception as e:
#         #     print 'Exception is %s' %e
#
#     # ''' vip_active.csv 写入'''
#     # if not os.path.exists(csvSaveRootPath + r'\vip_active'):
#     #      os.makedirs(csvSaveRootPath + r'\vip_active')
#     # file_vip_active = csvSaveRootPath + r'\vip_active\vip_active.csv'
#     # with open(file_vip_active, 'w') as f:
#     #     f.write('')
#     # with open(file_vip_active, 'a') as f:
#     #     for option in options:
#     #         option.encode('gbk')
#     #         activity = option[:-11]
#     #         optionBeginDate = option[-10:]
#     #         f.write(option + ',' + activity + ',' + optionBeginDate + '\n')
